...
export function usePokemon(query) {
  ...
  const [{data, status, errors}, setStates]= useState({
    data: null,
    status: "idel",
    errors: null
  })
  useEffect(() => {
    setStates({data, errors: null, status: "pending" });
    fetch(`https://pokeapi.co/api/v2/pokemon/${query}`)
      ...
      .then(data => {
        setStates({ data, errors: null, status: "fulfilled" });
      })
      .catch(err => {
        setStates({ data: null, errors: [err], status: "rejected" });
      });
